...
class NNModelWrapper2():
    ...
    def createBaseNetwork(self, noOfAlgorithms):
        if (NNModelWrapper2.baseNetwork is None):
            net = nn.Sequential()
            net.add(
                nn.Dense(3*noOfAlgorithms+1, activation = "relu"),
                nn.Dense(3*noOfAlgorithms+1, activation = "relu"),
            ...
            NNModelWrapper2.baseNetwork = net
        return NNModelWrapper2.baseNetwork
...
'''
class NeuralNetworkAlgorithmSelector2(AlgorithmSelector):
    ...
    def __init__(self, algorithms):
        ...
        self._aucModel = NNModelWrapper2(noOfAlgorithms = self._noOfAlgorithms)
        self._lowestCost = None
        self._debugDisplayCounter = 0
    
    def calcAUC(self, scores, costs, startbudget):
        areaUnderScore = 0
        lastBestScore = None
        
        for (score, cost) in list( zip(scores, costs) ):
            areaUnderScore += lastBestScore * np.abs( cost )
            if ( lastBestScore is None ):
                lastBestScore = score
            elif ( lastBestScore < score ):
                lastBestScore = score         
        areaUnderScore += lastBestScore * np.abs( startbudget - np.sum(costs) )
        return areaUnderScore

    def updateModel(self, scores, costs, runs, budget):
        ...
        #in case there are too many permutations, we will not generate all but exit after a certain amount of time. The following in-place shuffle ensures that we are not stuck on evaluation the lowest index algorithms more often than other combinations.
        np.random.shuffle(algorithmIndexes)
        
        rootNode = Node("root")
        for indexOrder in permutations(algorithmIndexes):
            finalAUCForThisIndexOrder = self.calcAUC( scores = scores[indexOrder], costs = costs[indexOrder], startbudget = budget )
            currentNode = rootNode
            
            for index in indexOrder:
                childString = '/root/'+index
                for childNode in currentNode.children:
                    if childNode.name == childString:
                        currentNode = childNode
                        if (currentNode.value < finalAUCForThisIndexOrder):
                            currentNode.auc = finalAUCForThisIndexOrder
                        break
                if currentNode.name != childString:
                    currentNode = Node(childString, parent = currentNode, auc = finalAUCForThisIndexOrder)
                
            
        print(rootNode)
        #TODO: Derive input states and then update the prediction model to predict the best auc values given a specific state.
        state = np.concatenate((scores, costs, runs, [budget]), axis=0)
    ...
    def process(self, budget, callback):
        ...
        self.updateModel(scores.copy(), costs.copy(), runs.copy(), budget)
