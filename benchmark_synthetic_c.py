...
def benchmark_synthetic_c(selectors, alg_n = 100, problem_n = 10000, budgetfactor = 0.25):
    """
    ...
    There are clearly better and worse algorithms.
    ...
    algorithmQualities = np.random.uniform(size=(alg_n,2)) # for each algorithm, we draw two random variables that modify it's performance qualities.
    ...
    for p in problems:
        ...
        def callback(algorithm):
            ...
            simulatedScore = algorithmQualities[algorithm,0] * 10 * ( 1.0 - ( np.abs(p - algorithm) / alg_n ) )# 1-10, based on their distance, favoring further away algorithms
